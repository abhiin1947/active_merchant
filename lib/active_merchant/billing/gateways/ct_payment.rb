module ActiveMerchant #:nodoc:
  module Billing #:nodoc:
    class CtPaymentGateway < Gateway
      ...
      def commit(action, parameters, send_ack = true)
        ...
        if send_ack
          post = {
            TransactionNumber: split_authorization(final_response.authorization)[0]
          }
          commit('ack', post, false)
        end
        final_response
      end
      ...
      def authorization_from(response)
        ...
        "#{response['invoiceNumber']};#{response['token']}"
      end
